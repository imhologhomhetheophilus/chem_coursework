...
<div id="typing-indicator" class="text-sm text-gray-500 hidden">Typing...</div>
...
$(document).ready(function(){
    ...
    $("#message").on('input',function(){
        $.post('typing_status.php',{typing:1,chatWith:"<?php echo $chatWith; ?>"});
    });
    $("#message").on('blur',function(){
        $.post('typing_status.php',{typing:0,chatWith:"<?php echo $chatWith; ?>"});
    });

    function fetchTyping(){
        $.get('fetch_typing.php?user=<?php echo $chatWith; ?>',function(data){
            if(data==1) $("#typing-indicator").show();
    ...
    function checkUnread(){
        $.getJSON('fetch_unread.php',function(data){
            let unread=0;
            for(let u in data) unread+=data[u];
            if(unread>0) document.title=`(${unread}) New Messages!`;
